...
export interface RungInput { bp: string; percent: string }
...
export function manualPlan(input: ManualInputs): { plan: ManualPlanRequest | null; errors: Record<string, string>; total: string } {
  ...
  } else positive('singleTpBp', input.singleTpBp, '止盈距离')
  ...
  const plan: ManualPlanRequest = {
    ...
    stop_distance_bp: input.stopBp.trim(), leverage: input.leverage.trim(),
    size_pct: ratio(input.sizePct, '100'), client_ref: 'manual',
  }
  if (input.useLadder) plan.take_profit = input.rungs.map((r) => ({ pct: ratio(r.bp, '10000'), fraction: ratio(r.percent, '100') }))
  else plan.take_profit_pct = ratio(input.singleTpBp, '10000')
