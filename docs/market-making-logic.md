...
本文描述失败突破回踩策略的研究和执行契约；缠论中枢回踩作为独立策略实现，不与本策略混合。PAPER 默认开启，LIVE 只能在显式模式、账户对账和 ARM 后使用 Maker-only 执行器。
...
- PAPER 使用公开行情、信号和模拟退出；LIVE 通过 Binance 签名适配器提交 `LIMIT + GTX` Maker-only 订单，并由用户数据流和账户更新持续对账。真实交易仍需显式 LIVE、允许的 endpoint、账户核验和 ARM，不能由 PAPER 自动升级。
