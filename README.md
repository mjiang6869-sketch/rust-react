...
**引擎的行情源还没接进来**：撮合、成交模型、WebSocket 推送都是通的，但没有
行情喂给引擎。后果是——

- 图表**有数据**：它独立读币安公开行情（K 线历史走 REST，实时 K 线、盘口与成交流走推送
  `/api/v1/market/stream`），不依赖引擎
- **模拟盘不会成交**：引擎收不到行情，策略不会产生意图，下单也不会撮合

这两件事走的是两条独立的数据通路，所以要分开看。
